...
<body>
    ...
    <div class="container shadow p-5 my-5 bg-white ">
        ...
        <div class="m-2">
            <a href="./index.php" class="btn btn-success btn-sm ">NEW PIAT</a>
            </div>
            <div class="m-2">
                             ...
                             ///  echo json_encode($_POST);
                                if ($col_name == 'both') {
                                   // echo  $from_siap.'-'.$to_siap.'-'.$from_paid.'-'.$to_paid;
                                    $stmt = $pdo->prepare("SELECT * FROM public.ad_service_qr WHERE ba LIKE :ba AND ((csp_paid_date >= :from_paid AND csp_paid_date <= :to_paid)  OR (tarikh_siap >= :from_siap AND tarikh_siap <= :to_siap))   ORDER BY csp_paid_date DESC");
